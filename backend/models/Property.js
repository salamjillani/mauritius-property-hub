...
PropertySchema.pre('save', function (next) {
  if (this.location?.coordinates?.length === 2) {
    ...
    if (lat < -90 || lat > 90 || lng < -180 || lng > 180) {
      return next(new Error('Invalid coordinates: latitude must be -90 to 90, longitude -180 to 180'));
    }
  } else if (this.isModified('location.coordinates') && this.location.coordinates) {
    return next(new Error('Coordinates must contain exactly two numbers: [longitude, latitude]'));
  }
  next();
});
