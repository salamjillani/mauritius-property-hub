...
exports.createAgent = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.role !== "agent") {
    await User.findByIdAndUpdate(req.user.id, {
      role: "agent",
      avatarUrl: req.body.photoUrl || "default-avatar.jpg",
    });
  }
...
exports.requestAgencyLink = asyncHandler(async (req, res, next) => {
  ...
  if (!agent) {
    return next(
      new ErrorResponse(`Agent not found with id of ${agentId}`, 404)
    );
  }
  ...
  if (!agency) {
    return next(
      new ErrorResponse(`Agency not found with id of ${agencyId}`, 404)
    );
  }
  ...
  if (agency.approvalStatus !== "approved") {
    return next(
      new ErrorResponse(
        "Cannot link to an unapproved or ineligible agency",
        403
      )
    );
  }
  ...
  const existingRequest = agent.linkingRequests.find(
    (request) =>
      request.agency.toString() === agencyId && request.status === "pending"
  );

  if (existingRequest) {
    return next(
      new ErrorResponse(
        "A pending linking request already exists for this agency",
        400
      )
    );
  }
...
exports.approveAgencyLink = asyncHandler(async (req, res, next) => {
  ...
  if (agency.approvalStatus !== "approved") {
    return next(
      new ErrorResponse(
        "Cannot approve link to an unapproved or ineligible agency",
        403
      )
    );
  }
...
exports.requestLinkToAgency = asyncHandler(async (req, res, next) => {
  ...
  if (agency.approvalStatus !== "approved") {
    return next(new ErrorResponse("Cannot link to an unapproved agency", 400));
  }
  ...
  if (existingRequest) {
    return next(
      new ErrorResponse("A pending request already exists for this agency", 400)
    );
  }
  ...
  await Notification.create({
    user: agency.user,
    type: "agency_link_request",
    message: `Agent ${user.firstName} ${user.lastName} has requested to link with your agency.`,
  });
...
exports.approveAgentLink = asyncHandler(async (req, res, next) => {
  ...
  if (request.agency.toString() !== agency._id.toString()) {
    return next(
      new ErrorResponse("Request does not belong to this agency", 403)
    );
  }
  ...
  agent.linkingRequests = agent.linkingRequests.map((req) => {
...
exports.getLinkingRequests = asyncHandler(async (req, res, next) => {
  ...
  if (!agency) {
    return next(
      new ErrorResponse(`Agency not found for user ${req.user.id}`, 404)
    );
  }
